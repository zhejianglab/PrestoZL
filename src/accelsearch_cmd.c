...
{
    fprintf(stderr, "%s",
            "   [-batchsize batchsize] [-ncpus ncpus] [-lobin lobin] [-numharm numharm] [-zmax zmax] [-wmax wmax] [-sigma sigma] [-rlo rlo] [-rhi rhi] [-flo flo] [-fhi fhi] [-inmem] [-photon] [-median] [-locpow] [-zaplist zaplist] [-baryv baryv] [-otheropt] [-noharmpolish] [-noharmremove] [--] infile ...\n");
    fprintf(stderr, "%s",
            "      Search an FFT or short time series for pulsars using a Fourier domain acceleration search with harmonic summing.\n");
    fprintf(stderr, "%s",
            "           -ncpus: Number of processors to use with OpenMP\n");
    fprintf(stderr, "%s",
            "           -batchsize: Number of batchsize to use\n");
...
Cmdline *parseCmdline(int argc, char **argv)
{
    ...
    for (i = 1, cmd.argc = 1; i < argc; i++)
    {
        ...
        if (0 == strcmp("-batchsize", argv[i]))
        {
            ...
            cmd.batchsizeC = i - keep;
            checkIntHigher("-batchsize", &cmd.batchsize, cmd.batchsizeC, 1);
            continue;
        }
