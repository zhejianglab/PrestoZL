...
void gratio2(double *a, double *x, double *ans, double *qans, int *ind)
{
  ...
  static double wk[20];
  static double e, acc, e0, x0, t1, u, r, twoa, amn, t, sum, c;
...
S130:
  ...
  *x = exp((w + *x - log(1.0e0 + *x / ap1 * (1.0e0 + *x / ap2))) / *a);
  *x = exp((w + *x -
            log(1.0e0 + *x / ap1 * (1.0e0 + *x / ap2 * (1.0e0 + *x / ap3)))) /
           *a);
  xn = *x;
  if (xn > 1.e-2 * ap1)
    goto S140;
